package tree.binarySearchTree.alg.balance_sol_1;
...
// Solution 1 : 노드들 하나씩 돌면서 자식 노드를 재귀 호출하고, 서브 트리의 길이를 측정하는 함수를 만들어서 양쪽 서브 
// 트리의 길이를 비교하다가 두 개의 서브 트리 길이가 1초과가 발생하면 false를 반환한다.
// 문제점: 한 번 노드가 호출될 때마다, getHeight 메서드가 해당 노드에서부터 양쪽으로 서브 트리 마지막 까지
// 돌면서 길이를 제는데, 노드가 호출 될 때마다 매번 다시 가서 길이를 젠다.
// 시간 복잡도: O(n log n)
...
// BinarySearchTree_Balance_Sol_1 클래스
...
}// BinarySearchTree_Balance_Sol_1 클래스 종료