...
밀가루가 떨어지지 않고 공장을 운영하기 위해서 최소한 몇 번 해외 공장으로부터 밀가루를 공급받아야
하는지를 return 하도록 solution 함수를 완성하세요.

dates[i]에는 i번째 공급 가능일이 들어있으며, supplies[i]에는 dates[i]
 날짜에 공급 가능한 밀가루 수량이 들어 있습니다.
...
// 솔루션: priorityQueue (우선 순위 큐)

class Ramen_Factory_Solution_01 {
    public int solution(int stock, int[] dates, int[] supplies, int k) {
    	 
    	 int answer = 0;
         // 우선 순위 큐 선언 및 생성
    	 // 우선 순위 큐에서 순위가 가장 높은 것을 큰 수로 함
         ...
         // 0일부터 k일까지 반복
         for (int i = 0; i < k; i++) {
         // date 배열에 있는 날짜와 i가 같을 때
        	 if((index < dates.length) && (i == dates[index])) {
             
        		 // 우선순위 큐에 밀가루 추가, index 1씩 증가
        		 priorityQueue.add(supplies[index++]);
        	 }
             
        	 // stock이 0일 때
        	 if(stock == 0) {
                 // 우선순위 큐에서 밀가루양을 꺼내기 -> 가장 supplies 값이 큰 것이 나온다.
            	 stock += priorityQueue.poll();
                 answer++;
             
             }
             
        	 // 하루가 지날때 마다 stock에서 밀가루양 1 감소
             stock -= 1;
         }
...
public class Ramen_Factory_Sol_01 {
...
*/
	...
	public static void main(String[] args) {
		...
		Ramen_Factory_Solution_01 sol = new Ramen_Factory_Solution_01();
