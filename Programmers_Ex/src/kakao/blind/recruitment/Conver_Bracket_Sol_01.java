...
class Conver_Bracket_solution_01 {
	...
	public int distinguishBalnceBracket(String str) {
	...
	public boolean decideRightBracket(String str) {
	...
	// 괄호 역방향으로 바꾸는 메서드: 기존에 있던 문자열을 역방향으로 바꾸는 새로운 string 생성
	public String converRightBracket(String str) {
	...
	// w 문자열에서 u,v 분리하는 메서드
	public String separateString(String str) {

		// 입력된 문자열
		String w = str;
		// 균형잡힌 문자열(더이상 분리 안되는 것)
		String u = "";
		// u를 제외한 나머지 문자열, 빈 문자열이 될 수 있음.
		String v = "";
		...
		u = w.substring(0, distinguishBalnceBracket(w) + 1);

		if (distinguishBalnceBracket(w) + 1 < w.length()) {

			v = w.substring(distinguishBalnceBracket(w) + 1);

		} else {
			v = "";
		}

		if (decideRightBracket(u)) {
			return u + separateString(v);
		} else {
			result += result + "(" + separateString(v) + ")" + converRightBracket(u.substring(1, u.length() - 1));
		}
