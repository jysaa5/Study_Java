...
class String_Compression_Solution_02 {
	   ...
	   // n: 자르는 단위 
	   // repeat: 반복횟수
	   // getSplitedLength 메서드: 문자열 압축
	    public String getSplitedLength(String s, int n, int repeat){
	        
	    	// 자르는 단위가 압축할 문자열 길이보다 길어질 때.
	    	if(s.length() < n) {
	    		return s;
	    	}
	        ...
	        // 처음 자른 문자열과 뒤에 부분의 시작부분이 일치하지 않을때.
	        if(!postString.startsWith(preString)){
	        
	        	// 반복이 안될때: 잘린 문자열이 한 번만 있을 때: 반복횟수 적지 않음
	        	if(repeat ==1) {
	        		return result += preString + getSplitedLength(postString, n, 1);
	        	}
	        	// 반복 횟수를 문자열로 변환 + 자른 문자열 + 뒷 부분 재귀함수 호출
	        	return result += Integer.toString(repeat) + preString + getSplitedLength(postString, n, 1);
	        }

	        // 처음 자른 문자열이 뒤에 부분과 시작 부분이 일치할 때, 재귀함수 호출 
